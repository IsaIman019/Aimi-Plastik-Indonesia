...
<head>
    ...
    <title>Aimi Packaging - Solusi Kemasan | @yield('title')</title>
            ...
            <div class="flex justify-between h-20">
                ...
                <div class="flex items-center">
                    <a href="{{ route('home') }}" class="flex-shrink-0 flex items-center gap-3 group">
                        <img src="{{ asset('images/Aimi.png') }}" alt="Logo Aimi"
                            class="h-12 w-auto object-contain group-hover:opacity-80 transition">
                        <div class="flex flex-col">
                            <span class="font-bold text-xl text-gray-900 tracking-tight leading-none">Aimi<span
                                    class="text-orange-600">Packaging</span></span>
                            <span class="text-[10px] text-gray-400 font-medium tracking-wider uppercase">Solusi
                                Kemasan</span>
                        </div>
                    </a>
                </div>
                ...
                <div class="hidden md:flex items-center space-x-8">
                    <a href="{{ route('home') }}"
                        class="text-sm font-bold {{ request()->routeIs('home') ? 'text-orange-600' : 'text-gray-600 hover:text-orange-600' }} transition">Beranda</a>

                    <a href="{{ route('about') }}"
                        class="text-sm font-bold {{ request()->routeIs('about') ? 'text-orange-600' : 'text-gray-600 hover:text-orange-600' }} transition">Tentang
                        Kami</a>

                    <a href="{{ route('artikel.index') }}"
                        class="text-sm font-bold {{ request()->routeIs('artikel.*') ? 'text-orange-600' : 'text-gray-600 hover:text-orange-600' }} transition">Artikel</a>

                    {{-- MENU BARU: DROPDOWN MARKETPLACE --}}
                    <div class="relative group">
                        <button
                            class="flex items-center gap-1 text-sm font-bold text-gray-600 hover:text-orange-600 transition focus:outline-none">
                            Marketplace
                        ...
                        {{-- Isi Dropdown --}}
                        <div
                            class="absolute left-1/2 -translate-x-1/2 mt-0 w-48 bg-white rounded-xl shadow-xl border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top pt-4 z-50">
                            <div class="p-2 bg-white rounded-xl">

                                {{-- Shopee --}}
                                <a href="https://shopee.co.id/aimiofficial" target="_blank"
                                    class="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-orange-50 group/item transition">
                                    <svg class="w-5 h-5 text-orange-500 group-hover/item:scale-110 transition"
                                    ...
                                    <span
                                        class="text-sm font-bold text-gray-700 group-hover/item:text-orange-600">Shopee</span>
                                </a>

                                {{-- TikTok --}}
                                <a href="https://www.tiktok.com/@aimiplastik.id" target="_blank"
                                    class="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-gray-100 group/item transition">
                                    <svg class="w-5 h-5 text-black group-hover/item:scale-110 transition"
                                        viewBox="0 0 24 24" fill="currentColor">
                                        <path
                                    ...
                                    <span
                                        class="text-sm font-bold text-gray-700 group-hover/item:text-black">TikTok</span>
                                </a>

                                {{-- Instagram --}}
                                <a href="https://www.instagram.com/aimiplastik.id" target="_blank"
                                    class="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-pink-50 group/item transition">
                                    <svg class="w-5 h-5 text-pink-600 group-hover/item:scale-110 transition"
                                        viewBox="0 0 24 24" fill="currentColor">
                                        <path
                    ...
                    <a href="{{ route('home') }}#katalog"
                        class="text-sm font-bold text-gray-600 hover:text-orange-600 transition">Katalog</a>

                    <a href="{{ route('home') }}#kontak"
                        class="text-sm font-bold text-gray-600 hover:text-orange-600 transition">Kontak</a>
                </div>

                <div class="flex items-center gap-3">
                    <button
                        class="p-2 text-gray-400 hover:text-gray-600 transition rounded-full hover:bg-gray-100 hidden sm:block">
                    ...
                    <div class="relative group cursor-pointer ml-1">
                        ...
                        <div
                            ...
                            <form action="{{ route('logout') }}" method="POST">
                                @csrf
                                <button type="submit"
                                    class="w-full text-left px-4 py-3 text-sm text-red-600 hover:bg-red-50 rounded-b-xl font-medium transition border-t border-gray-50">Logout</button>
                            </form>
                        </div>
                    </div>
                    @else
                    <a href="{{ route('login') }}"
                        class="text-sm font-bold text-gray-600 hover:text-orange-600 transition px-3">Masuk</a>
                    <a href="{{ route('register') }}"
                        class="px-5 py-2.5 text-sm font-bold text-white bg-orange-600 rounded-full hover:bg-orange-700 transition shadow-lg shadow-orange-200">Daftar</a>
                    @endauth
                </div>
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
                <div class="col-span-1 md:col-span-2">
                    <img src="{{ asset('images/Aimi.png') }}" alt="Logo Aimi"
                        class="h-16 w-auto mb-4 bg-white rounded-lg p-1">
                    <p class="text-gray-400 text-sm leading-relaxed max-w-sm">
                        Partner terpercaya untuk kebutuhan packing bisnis Anda. Menyediakan lakban, bubble wrap, dan
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-sm text-gray-400">
                        ...
                        <li><a href="{{ route('about') }}" class="hover:text-orange-500 transition">Tentang Kami</a>
                        </li>
                        <li><a href="{{ route('artikel.index') }}" class="hover:text-orange-500 transition">Artikel</a>
                        </li>
