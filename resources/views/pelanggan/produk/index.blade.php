    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col lg:flex-row gap-4 mb-10">
            ...
            <a href="{{ route('pelanggan.kategori.all') }}"
                class="w-full lg:w-1/6 bg-blue-50 rounded-xl border border-blue-100 flex flex-col items-center justify-center p-6 text-center group hover:bg-blue-100 transition relative overflow-hidden min-h-[180px]">
                <div
                    class="absolute -top-10 -left-10 w-32 h-32 bg-blue-100 rounded-full opacity-50 blur-xl group-hover:bg-blue-200 transition">
                </div>
                <div class="relative z-10">
                    ...
                    <svg class="w-5 h-5 text-blue-600 mx-auto transform group-hover:translate-x-1 transition"
            ...
            <div class="flex-1 bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                <div
                    class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 divide-y sm:divide-y-0 md:divide-x md:divide-y divide-gray-100">
                    @foreach($kategoris->take(8) as $index => $kategori)
                    <a href="{{ route('pelanggan.kategori.detail', $kategori->id) }}"
                        class="group p-4 flex items-center gap-3 hover:bg-gray-50 transition h-full {{ $index >= 4 ? 'border-t border-gray-100' : '' }}">
                        ...
                        <div class="flex-shrink-0 w-12 h-12 flex items-center justify-center">
                            ...
                            @else
                            <svg class="w-8 h-8 text-gray-300 group-hover:text-orange-500 transition" fill="none"
                        ...
                        <div>
                            <h4
                                class="text-sm font-bold text-gray-800 group-hover:text-orange-600 transition leading-tight line-clamp-2">
                                {{ $kategori->nama }}
                            </h4>
        ...
        if($produk->stok > 0) {
        $stockBadge = '<span
            class="text-[10px] font-medium text-green-700 bg-green-50 px-2 py-0.5 rounded border border-green-100">Stok:
            '.$produk->stok.'</span>';
        } else {
        $stockBadge = '<span
            class="text-[10px] font-medium text-red-700 bg-red-50 px-2 py-0.5 rounded border border-red-100">Habis</span>';
        }
        ...
        <div
            ...
            <a href="'.route('pelanggan.produk.show', $produk->id).'">
                ...
                <div class="relative w-full pt-[100%] bg-white rounded-t-xl overflow-hidden">';
                    ...
                    if($showBadge) {
                    $html .= '<span
                        class="absolute top-0 left-0 bg-[#6f7ecc] text-white text-[10px] font-bold px-3 py-1 rounded-br-lg z-10">NEW</span>';
                    }

                    // Image
                    if($produk->image) {
                    $html .= '<img src="'.asset('storage/' . $produk->image).'"
                        class="absolute inset-0 w-full h-full object-contain p-6 group-hover:scale-105 transition duration-500">';
                    } else {
                    $html .= '<div class="absolute inset-0 flex items-center justify-center text-gray-300">
                    ...
                    // Tombol Keranjang (Hanya muncul jika stok > 0)
                    if($produk->stok > 0) {
                    $html .= '<div
                ...
                <div class="p-4">
                    ...
                    <div class="flex items-center justify-between mb-2">
                        <span class="text-[#2c3e8c] font-bold text-base">Rp '.number_format($produk->harga, 0, ',',
                            '.').'</span>
                        '.$stockBadge.'
                    </div>
                </div>
        ...
        <div class="mb-12">
            ...
            <div class="relative group/slider">
                {{-- Arrow Left --}}
                <button
                    class="absolute left-0 top-1/2 -translate-y-1/2 -ml-4 z-20 w-10 h-10 bg-white rounded-full shadow-lg border border-gray-100 flex items-center justify-center text-blue-900 hover:bg-blue-50 opacity-0 group-hover/slider:opacity-100 transition-opacity">
                ...
                {{-- Arrow Right --}}
                <button
                    class="absolute right-0 top-1/2 -translate-y-1/2 -mr-4 z-20 w-10 h-10 bg-white rounded-full shadow-lg border border-gray-100 flex items-center justify-center text-blue-900 hover:bg-blue-50 opacity-0 group-hover/slider:opacity-100 transition-opacity">
        ...
        <div class="mb-12">
            ...
            <div class="relative group/slider-best">
                {{-- Arrow Left --}}
                <button
                    class="absolute left-0 top-1/2 -translate-y-1/2 -ml-4 z-20 w-10 h-10 bg-white rounded-full shadow-lg border border-gray-100 flex items-center justify-center text-blue-900 hover:bg-blue-50 opacity-0 group-hover/slider-best:opacity-100 transition-opacity">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7">
                        </path>
                    </svg>
                ...
                {{-- Arrow Right --}}
                <button
                    class="absolute right-0 top-1/2 -translate-y-1/2 -mr-4 z-20 w-10 h-10 bg-white rounded-full shadow-lg border border-gray-100 flex items-center justify-center text-blue-900 hover:bg-blue-50 opacity-0 group-hover/slider-best:opacity-100 transition-opacity">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                    </svg>
                </button>
