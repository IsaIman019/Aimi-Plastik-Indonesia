...
<div class="bg-white py-12 min-h-screen">
    <div class="max-w-7xl mx-auto px-6 font-sans">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8 mb-12">
            <div class="lg:col-span-3">
                ...
                <article class="group cursor-pointer relative rounded-xl overflow-hidden shadow-lg h-[450px]">
                    <a href="{{ route('artikel.show', $featuredArticle->id) }}">
                        <img src="{{ url('storage/' . $featuredArticle->gambar) }}" alt="{{ $featuredArticle->judul }}"
                        ...
                        <div class="absolute inset-0 flex flex-col justify-end p-6 md:p-8">
                            <div class="bg-white px-4 py-1.5 rounded-full shadow-md w-fit mb-4">
                                <span class="text-slate-500 text-[11px] font-bold">
                                    {{ $featuredArticle->created_at->format('d F Y') }}
                                </span>
                            </div>
                            ...
                            <p class="text-gray-200 leading-relaxed text-sm md:text-base max-w-4xl line-clamp-2">
                                {{ \Illuminate\Support\Str::limit(strip_tags($featuredArticle->konten), 200) }}
                            </p>
                        </div>
                    </a>
                </article>
                @else
                <!-- Default jika tidak ada featured article -->
                <article class="group cursor-pointer relative rounded-xl overflow-hidden shadow-lg h-[450px]">
                    ...
                    <div class="absolute inset-0 flex flex-col justify-end p-6 md:p-8">
                        ...
                        <p class="text-gray-200 leading-relaxed text-sm md:text-base max-w-4xl line-clamp-2">
                            Panduan lengkap tentang Assembly Point atau titik kumpul darurat.
                        </p>
                    </div>
                </article>
                @endif
            ...
            <div class="lg:col-span-1">
                ...
                <div class="flex flex-col">
                    ...
                    @endforeach
                    @else
                    <!-- Default popular articles jika tidak ada data -->
                    @php
                    $defaultPopuler = [
                    ...
                    ['title' => 'Cermin Cembung (Convex Mirror) untuk Jalan Raya', 'date' => '24 March 2025', 'img' =>
                    'https://images.unsplash.com/photo-1582139329536-e7284fece509?w=400'],
        ...
        <!-- Articles by Category (PER KATEGORI) -->
        @if(isset($articlesByCategory) && count($articlesByCategory) > 0)
        @foreach($articlesByCategory as $categoryData)
        <div class="mb-12">
            <!-- Category Header -->
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h3 class="text-2xl font-bold text-slate-800">
                        {{ $categoryData['kategori']->nama ?? 'Kategori ' . $categoryData['kategori']->id }}
                    </h3>
                    @if(isset($categoryData['kategori']->deskripsi))
                    <p class="text-slate-500 text-sm mt-1">{{ $categoryData['kategori']->deskripsi }}</p>
                    @endif
                </div>
                <a href="/artikel/kategori/{{ $categoryData['kategori']->id }}"
                    ...
                    <a href="{{ route('artikel.show', $article->id) }}">
                        <div class="relative mb-8">
                            <div class="rounded-xl overflow-hidden shadow-sm aspect-video">
                                <img src="{{ url('storage/' . $article->gambar) }}" alt="{{ $article->judul }}"
                                    class="w-full h-full object-cover transition duration-500 group-hover:scale-110">
                            </div>

                            <div
                                class="absolute -bottom-4 left-6 bg-white px-4 py-1.5 rounded-full shadow-md border border-gray-50 z-10">
                                <span class="text-slate-400 text-[10px] font-bold whitespace-nowrap uppercase">
                        ...
                        <div class="px-2">
                            ...
                            <p class="text-slate-400 text-xs leading-relaxed line-clamp-2">
                                {{ \Illuminate\Support\Str::limit(strip_tags($article->konten), 100) }}
                            </p>

                            <!-- Category Badge -->
                            <div class="mt-2">
                                <span class="inline-block px-2 py-1 bg-red-50 text-red-600 text-xs rounded">
                                    {{ $categoryData['kategori']->nama ?? 'Kategori ' . $categoryData['kategori']->id }}
                                </span>
                            </div>
                        </div>
        ...
        <!-- All Articles dengan Load More -->
        @if(isset($allArtikel) && $allArtikel->count() > 0)
        <div x-data="{ limit: 8, totalData: {{ $allArtikel->count() }} }" class="mb-12">
            <h3 class="text-2xl font-bold text-slate-800 mb-6">Semua Artikel</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
                ...
                <article x-show="{{ $index }} < limit" x-transition:enter="transition ease-out duration-500"
                    ...
                    <a href="{{ route('artikel.show', $artikel->id) }}">
                        <div class="relative mb-8">
                            <div class="rounded-xl overflow-hidden shadow-sm aspect-video">
                                <img src="{{ url('storage/' . $artikel->gambar) }}" alt="{{ $artikel->judul }}"
                                    class="w-full h-full object-cover transition duration-500 group-hover:scale-110">
                            </div>
